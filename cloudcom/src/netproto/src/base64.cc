...
#include "netproto/include/base64.h"
...
namespace npp
{
	...
	*/
    ...
    //------------------------------------------------------------
    bool Base64::encode( const std::string& srcData, MemoryDataStream& encodedData )
    {
        return encode( ( const u8* )srcData.c_str(), srcData.size(), encodedData );
    }

    //------------------------------------------------------------
    bool Base64::encode( const std::string& srcData, std::string& encodedData )
    {
        MemoryDataStream ds;

        if ( encode( ( const u8* )srcData.c_str(), srcData.size(), ds ) )
        {
            encodedData = std::string( ( const char* )ds.getCache(), ds.getSize() );
            return true;
        }
    ...
    //------------------------------------------------------------
    bool Base64::encode( const MemoryDataStream& srcData, std::string& encodedData )
    {
        MemoryDataStream ds;
    ...
    //------------------------------------------------------------
    bool Base64::decode( const MemoryDataStream& srcData, std::string& decodedData )
    {
        MemoryDataStream ds;
        ...
        decodedData = std::string( ( const char* )ds.getCache(), ds.getSize() );
        return true;
    }

    //------------------------------------------------------------
    bool Base64::decode( const std::string& srcData, MemoryDataStream& decodedData )
    {
        return decode( ( const u8* )srcData.c_str(), srcData.size(), decodedData );
    }

    //------------------------------------------------------------
    bool Base64::decode( const std::string& srcData, std::string& decodedData )
    {
        MemoryDataStream ds;
