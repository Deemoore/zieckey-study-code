...
namespace osl
{
    ...
    inline T* get_ptr_in_map( const npp::Map<K, T*, N>& iHMap, const K& key )
    {
        // Typedef here avoids warnings because of new c++ naming rules.
        typedef typename npp::Map<K, T*, N>::const_iterator map_iter;
    ...
    template <typename K, typename T, typename N>
    inline bool is_in_map( const npp::Map<K, T, N>& iHMap, const K& key )
    {
        typedef typename npp::Map<K, T, N>::const_iterator map_iter;
    ...
    template <typename K, typename T, typename N>
    inline T get_if_there( const npp::Map<K, T, N>& iHMap, const K& key, T default_value )
    {
        // Typedef here avoids warnings because of new c++ naming rules.
        typedef typename npp::Map<K, T, N>::const_iterator map_iter;
        map_iter iter = iHMap.find( key );
    ...
    // Example:
    //  for (npp::Vector<T>::iterator iter = mList.begin(); iter != mList.end(); )
    //  {
    ...
    inline Iter vector_replace_with_last( npp::Vector<T, N>& invec, Iter iter )
    {
        typename npp::Vector<T, N>::iterator last = invec.end();
        --last;
    ...
    inline bool vector_replace_with_last( npp::Vector<T, N>& invec, const T& val )
    {
        typename npp::Vector<T, N>::iterator iter = std::find( invec.begin(), invec.end(), val );
    ...
    template <typename T, typename N>
    inline T* vector_append( npp::Vector<T, N>& invec, int nSize )
    {
    ...
    */
    ...
    void for_each_process2nd( const MAP & realActorMap , const Functor& funWork )
    {
        ...
        for_each( realActorMap.begin() , realActorMap.end()
            , npp::h_compose1( funWork , npp::h_select2nd<typename MAP::value_type>() )
            );
    }
#define forEachMapProcess2nd for_each_process2nd //for compatible old code
    ...
    void for_each_process1st( const MAP & realActorMap , const Functor& funWork )
    {
        ...
        //         }
        for_each( realActorMap.begin() , realActorMap.end()
            , npp::h_compose1( funWork , npp::h_select1st<typename MAP::value_type>() )
            );
    }
