...
#ifndef QOSLIB_LINUX_LIST_
#define QOSLIB_LINUX_LIST_
...
namespace npp
{
    ...
    template < typename _Tp
     = npp::STLList<_Tp> >
    class std::list : private std::list<_Tp , npp::HSTLAllocator<_Tp> >
    {
        typedef std::list<_Tp , npp::HSTLAllocator<_Tp> > BaseClass;
        size_t m_list_size;
    public:
        // type
    ...
    public:
        ...
        */
        explicit std::list( const allocator_type& _Al = allocator_type() )
            : BaseClass( _Al )
            , m_list_size( 0 )
        {
        }
        ...
        */
        std::list( size_type _Count, const _Tp& _Val, const allocator_type& _Al = allocator_type() )
        ...
        /**
        *  @brief  %std::list copy constructor.
        *  @param  x  A %list of identical element and allocator types.
        *
        *  The newly-created %list uses a copy of the allocation object used
        *  by @a x.
        */
        std::list( const std::list& __x )
        ...
        */
        ...
        std::list( InputIterator _First, InputIterator _Last, const allocator_type& _Al = allocator_type() )
            : m_list_size( 0 )
            , BaseClass( _Al )
        {
        ...
        /**
        *  @brief  %std::list assignment operator.
        *  @param  x  A %list of identical element and allocator types.
        *
        *  All the elements of @a x are copied, but unlike the copy
        *  constructor, the allocator object is not copied.
        */
        std::list& operator=( const std::list& __x )
        {
        ...
        *  list.
        */
        void splice( iterator __position, std::list& __x )
        {
            if ( !__x.empty() )
            {
        ...
        */
        void splice( iterator __position, std::list& __x, iterator __i )
        {
            if ( __i == __x.end() )
            {
                return;
        ...
        */
        void splice( iterator __position, std::list& __x , iterator __first, iterator __last )
        {
            if ( __first != __last )
            {
    ...
    // lists
    H_DEF_LIST( npp::s8,  S8List );
    H_DEF_LIST( npp::u8,  U8List );
    H_DEF_LIST( npp::s16, S16List );
    H_DEF_LIST( npp::u16, U16List );
    H_DEF_LIST( npp::s32, S32List );
    H_DEF_LIST( npp::u32, U32List );
    H_DEF_LIST( npp::s64, S64List );
    H_DEF_LIST( npp::u64, U64List );
	H_DEF_LIST( npp::f32, F32List );
	H_DEF_LIST( npp::f64, F64List );

    H_DEF_LIST( void*, VoidList );
