...
{	
	...
	m_bitmapHeader.GetBitmap(&bmpHeader);
	//dc.StretchBlt(rc.left, rc.top, rc.right-rc.left, rc.bottom-rc.top, dcTemp, 0, 0, bmpHeader.bmWidth, bmpHeader.bmHeight, SRCCOPY);
	...
	for(int i=0; i<nItems; ++i)
	{
		...
		//dc.StretchBlt(rcItem.left, rcItem.top, rcItem.right-rcItem.left, rcItem.bottom-rcItem.top, dcTemp, 0, 0, bmpHeader.bmWidth, bmpHeader.bmHeight, SRCCOPY);
...
CListViewCtrlEx::CListViewCtrlEx( void )
{
	...
	m_bitmapExpand.Attach( BkBmpPool::GetBitmap(IDB_BITMAP_LISTCTRL_EXPAND) );
	m_bitmapRadio.Attach( BkBmpPool::GetBitmap(IDB_BITMAP_LISTCTRL_RADIO) );

	m_bitmapSelectedRow.Attach(BkBmpPool::GetBitmap(IDB_BITMAP_LISTCTRL_SELECTED_ROW));
	m_bitmapNormalRow.Attach(BkBmpPool::GetBitmap(IDB_BITMAP_LISTCTRL_NORMAL_ROW));
	m_bitmapHoverRow.Attach(BkBmpPool::GetBitmap(IDB_BITMAP_LISTCTRL_HOVER_ROW));
...
void CListViewCtrlEx::_DrawNormalItem( LPDRAWITEMSTRUCT lpdis, const TListItem *pItem )
{
	...
	if ( bSelect )
	{
		...
		dcTemp.CreateCompatibleDC( dc );
		HBITMAP hOldBitmap = dcTemp.SelectBitmap(m_bitmapSelectedRow);
		BITMAP bmpSelectedRow;
	
		m_bitmapSelectedRow.GetBitmap(&bmpSelectedRow);

		dc.StretchBlt(rc.left, rc.top, 4, rc.bottom-rc.top, dcTemp, 0, 0, 4, bmpSelectedRow.bmHeight, SRCCOPY);
		dc.StretchBlt(rc.left+4, rc.top, rc.right-rc.left-4*2, rc.bottom-rc.top, dcTemp, 4, 0, bmpSelectedRow.bmWidth-4*2, bmpSelectedRow.bmHeight, SRCCOPY);
		dc.StretchBlt(rc.right-rc.left-4, rc.top, 4, rc.bottom-rc.top, dcTemp, bmpSelectedRow.bmWidth-4, 0, 4, bmpSelectedRow.bmHeight, SRCCOPY);
		dcTemp.SelectBitmap(hOldBitmap);

	}
	else
	{
		if(pItem->bScanFinished)
		{
			RECT rc = lpdis->rcItem;
			CDC dcTemp;

			dcTemp.CreateCompatibleDC( dc );
			HBITMAP hOldBitmap = dcTemp.SelectBitmap(m_bitmapNormalRow);
			BITMAP bmpNormalRow;

			m_bitmapNormalRow.GetBitmap(&bmpNormalRow);

			dc.StretchBlt(rc.left, rc.top, 4, rc.bottom-rc.top-1, dcTemp, 0, 0, 4, bmpNormalRow.bmHeight, SRCCOPY);
			dc.StretchBlt(rc.left+4, rc.top, rc.right-rc.left-4*2, rc.bottom-rc.top-1, dcTemp, 4, 0, bmpNormalRow.bmWidth-4*2, bmpNormalRow.bmHeight, SRCCOPY);
			dc.StretchBlt(rc.right-rc.left-4, rc.top, 4, rc.bottom-rc.top-1, dcTemp, bmpNormalRow.bmWidth-4, 0, 4, bmpNormalRow.bmHeight, SRCCOPY);
			dcTemp.SelectBitmap(hOldBitmap);
		}
		else
		{
