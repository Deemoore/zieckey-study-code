...
class IBkCtrlFactory
{
public:
	virtual CBkWindow* CreateControl(LPCSTR pszName) = 0;
};

typedef CAtlArray<IBkCtrlFactory*> __BkCtrlFactoryPool;
...
protected:
	...
	// 散沙添加，用于支持扩展控件
protected:
	static __BkCtrlFactoryPool m_sCtrlFactory;

public:
	static void RegistCtrlFactory(IBkCtrlFactory* pFactory)
	{
		for (int i=0; i<m_sCtrlFactory.GetCount(); i++)
		{
			IBkCtrlFactory* p = m_sCtrlFactory[i];
			if (p == pFactory) return;
		}

		m_sCtrlFactory.Add(pFactory);
	}

	// 删除皮肤工厂
	static void RemoveCtrlFactory(IBkCtrlFactory* pFactory)
	{
		for (int i=0; i<m_sCtrlFactory.GetCount(); i++)
		{
			IBkCtrlFactory* p = m_sCtrlFactory[i];
...
__declspec(selectany) __BkCtrlFactoryPool CBkPanel::m_sCtrlFactory;
...
//         return pNewWindow;
	...
	for (int i=0; i<m_sCtrlFactory.GetCount(); i++)
	{
		pNewWindow = m_sCtrlFactory[i]->CreateControl(lpszName);
