...
class CListComboEx : public CWindowImpl< CListComboEx, CComboBox >
{
public:
	CListComboEx() : m_wndEditCtrl( this, 1 )
	{
		m_nItem = NULL_ITEM;
...
public:
	BOOL Create( HWND hWndParent, int nItem, int nSubItem, CRect& rcRect, UINT nFlags, LPCTSTR lpszItemText, BOOL bShowThemed, CListArray < CString >& aComboList )
	{
		...
		if ( CWindowImpl< CListComboEx, CComboBox >::Create( hWndParent, CRect( ( ( dwStyle & CBS_DROPDOWNLIST ) == CBS_DROPDOWNLIST ) ? rcRect.left + 3 : rcRect.left + 1, rcRect.top, rcRect.right, rcRect.bottom + ( 6 * rcRect.Height() ) ), NULL, dwStyle ) == NULL )
			return FALSE;
