...
namespace osl
{
    ...
    //----------------------------------------------------------
    StringA OS::getOperationSystemVersion()
    {
...
//				}
    ...
    //----------------------------------------------------------
    bool OS::copyToClipboard( const StringA& strText )
    {
    ...
    //----------------------------------------------------------
    StringA OS::getTextFromClipboard()
    {
	...
	//----------------------------------------------------------
	StringA OS::getDisplayDriverName()
	{
		HRESULT hr;
		f64 fMHz = 0;
    ...
    void OS::setProcessAffinity( osl::u32 nProcessorMask )
    {
        // validate mask.
        {
            ...
            if( !::GetProcessAffinityMask( ::GetCurrentProcess(), ( PDWORD_PTR )&nCurProcessMask, ( PDWORD_PTR )&nSysMask ) )
            {
                ...
                // log
                {
                    ...
                    strTip << "OS::setProcessAffinity " << " GetProcessAffinityMask " << OS::getLastErrorString();
        ...
        if( !::SetProcessAffinityMask( ::GetCurrentProcess(), ( DWORD_PTR )nProcessorMask ) )
        {
            ...
            {
                osl::OStringStreamA strTip;
                strTip << "OS::setProcessAffinity " << " SetProcessAffinityMask " << OS::getLastErrorString();
                //H_LOG_NAME_ERROR( "sys", strTip.str() );
            }
        }
    }
    ...
    osl::u32 OS::getProcessAffinity( )
    {
        ...
        {
            osl::OStringStreamA strTip;
            strTip << "OS::getProcessAffinity " << " GetProcessAffinityMask " << OS::getLastErrorString();
            //H_LOG_NAME_ERROR( "sys", strTip.str() );
        }
