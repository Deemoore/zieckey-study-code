...
int OpenGLGame::drawGLScene( GLvoid ) /* 从这里开始进行所有的 嬷?*/
{
	...
	glBindTexture( GL_TEXTURE_2D, texture[m_nFilterIndex] ); // Select Our Texture
...
int OpenGLGame::loadGLTextures() /* Load Bitmaps And Convert To Textures */
{
	...
	if ( TextureImage[0] = loadBMPFile( "f:/a.bmp" ) )
	{
		Status = TRUE; // Set The Status To TRUE

		glGenTextures( E_TEXTURE_NUM, &texture[0] ); // Create The Texture
