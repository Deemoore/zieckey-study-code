...
int OpenGLGame::drawGLScene( GLvoid )
{
    ...
    {
        glPushMatrix();
        gltApplyActorTransform( &m_vSphereLocation[i] );
...
int OpenGLGame::initGL( GLvoid ) /* 此处开始对OpenGL进行所有设置 */
{
    ...
    {
        gltInitFrame( &m_vSphereLocation[iSphere] );  // Initialize the frame

        // Pick a random location between -20 and 20 at .1 increments
        m_vSphereLocation[iSphere].vLocation[0] = ( float )( ( rand() % 400 ) - 200 ) * 0.1f;
        m_vSphereLocation[iSphere].vLocation[1] = 1.0f;
        m_vSphereLocation[iSphere].vLocation[2] = ( float )( ( rand() % 400 ) - 200 ) * 0.1f;
    }
...
GLvoid OpenGLGame::resizeGLScene( GLsizei width, GLsizei height )
{
	...
	// 设置视口的大小
	gluPerspective( 35, ( GLfloat )width / ( GLfloat )height, 0.1f, 50.0 );
